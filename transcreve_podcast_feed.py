...
try:
    ...
    for i in feed.entries:
        ...
        if not os.path.exists('{}/{}'.format(sys.argv[2], nome_arq_saida)):
            ...
            open('/tmp/arq_media_podcast.{}'.format(nome_arq_saida), 'wb').write(r.content)
            print('Processando...')
            process = subprocess.Popen(['ffmpeg', '-loglevel', 'quiet', '-i',
                                        '/tmp/arq_media_podcast.{}'.format(nome_arq_saida),
            ...
            arq_saida.close()
            os.remove('/tmp/arq_media_podcast.{}'.format(nome_arq_saida))
finally:
